...
const SectionButtons = ({ editing, saving, onEdit, onSave, onCancel }) => (
    <div style={{ display: "flex", gap: "0.5rem" }}>
        {!editing ? (
            <button className="btn btn-secondary" onClick={onEdit} style={{ padding: "0.4rem 0.9rem", fontSize: "0.82rem" }}>Edit</button>
        ) : (
            <>
                <button className="btn btn-secondary" onClick={onCancel} disabled={saving} style={{ padding: "0.4rem 0.9rem", fontSize: "0.82rem" }}>Cancel</button>
...
export default function PlatformSettingsTab() {
    ...
    const saveToBackend = async (payload) => {
        const res = await authFetch("/admin/profile", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        return res.json();
    };
    ...
    if (loading) return <div style={{ display: "flex", justifyContent: "center", padding: "4rem" }}><Loader className="animate-spin" size={32} style={{ color: "var(--primary)" }} /></div>;
    ...
    return (
        <div className="animate-fade-in flex flex-col gap-8">
            ...
            <div className="glass-card" style={{ padding: "2rem" }}>
                ...
                <p style={{ marginTop: "1rem", fontSize: "0.82rem", color: "#92400e", padding: "0.75rem 1rem", background: "rgba(245,158,11,0.05)", borderRadius: "8px", border: "1px solid rgba(245,158,11,0.2)" }}>
                        ...
                        return (
                            <div key={cat} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1rem", borderRadius: "10px", background: "var(--surface)", border: "1px solid " + (isCustom ? "var(--primary)" : "var(--border)") }}>
                                <span style={{ fontSize: "0.88rem", fontWeight: 500, color: "var(--text)", flex: 1, marginRight: "0.5rem" }}>
                                    {cat}{isCustom && <span style={{ fontSize: "0.7rem", color: "var(--primary)", marginLeft: "6px", fontWeight: 700 }}>CUSTOM</span>}
                                </span>
            ...
            <div className="glass-card" style={{ padding: "1.75rem", background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.25)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.75rem" }}>
                    {["Click Edit to modify a section, then Save to persist changes to the database", "Platform fee and shipping changes apply immediately in consumer checkout order summary", "GST rate changes apply to sellers without a GST number and in consumer checkout"].map((note, i) => (
