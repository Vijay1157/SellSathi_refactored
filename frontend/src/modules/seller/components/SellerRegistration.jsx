      ...
      <div className="lg:w-1/2 p-8 lg:p-16 text-white flex flex-col justify-center relative min-h-[40vh] lg:min-h-screen" style={{ background: 'linear-gradient(135deg, #7B4DDB 0%, #6A3FD8 50%, #5A32C8 100%)' }}>
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-3 mb-12">
            <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center"><Store size={24} /></div>
            <div><h3 className="text-xl font-bold">Seller Center</h3><p className="text-white/60 text-sm">SellSathi for Business</p></div>
          </div>
          <h1 className="text-5xl lg:text-6xl font-bold leading-tight mb-8">Grow your business with SellSathi</h1>
          <p className="text-xl text-white/80 max-w-md mb-12">Reach millions of customers and scale your brand with powerful selling tools.</p>
        ...
        <div className="relative z-10 pt-12"><p className="text-sm text-white/60">By logging in, you agree to SellSathi's Seller <Link to="#" className="underline">Terms of Service</Link></p></div>
      </div>

      <div className="lg:w-1/2 bg-gray-50 flex items-center justify-center p-6 lg:p-12 overflow-y-auto lg:h-screen">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-5xl bg-white rounded-3xl shadow-xl p-8 lg:p-10 border border-gray-100 flex flex-col max-h-[90vh]">
              ...
              <div className="text-center">
                ...
                <div className="space-y-6">
                  <label className="block">
                    <div className="w-full h-48 rounded-3xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all group" style={{ borderColor: '#7B4DDB', backgroundColor: 'rgba(123, 77, 219, 0.05)' }}>
                      {isExtracting ? (
                        <div className="flex flex-col items-center"><Loader className="animate-spin mb-2" style={{ color: '#7B4DDB' }} size={32} /><p className="font-bold" style={{ color: '#7B4DDB' }}>Extracting...</p></div>
                      ) : (
                        <><div className="w-16 h-16 rounded-full bg-white flex items-center justify-center mb-4 shadow-sm group-hover:scale-110 transition-transform" style={{ color: '#7B4DDB' }}><Upload size={28} /></div><p className="font-bold" style={{ color: '#7B4DDB' }}>Upload Aadhaar Card</p><p className="text-xs text-gray-400 mt-2">Supports JPG, PNG, PDF</p></>
                      )}
                  ...
                  <button onClick={handleManualEntry} disabled={isExtracting} style={{ borderColor: '#7B4DDB', color: '#7B4DDB' }} className="w-full rounded-2xl border-2 py-4 font-bold hover:bg-opacity-5 transition-all disabled:opacity-50">Enter Details Manually</button>
                </div>
              </div>
            ) : (
              <div>
                ...
                <form className="space-y-6" onSubmit={handleSubmitSellerApplication}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2"><label className="text-sm font-bold text-gray-700 flex items-center gap-2"><User size={16} /> Full Name *</label><input type="text" value={formData.fullName} onChange={(e) => setFormData({ ...formData, fullName: e.target.value })} className="w-full rounded-xl border border-gray-200 px-4 py-3 focus:outline-none focus:ring-2 transition-all shadow-sm" style={{ '--tw-ring-color': '#7B4DDB33' }} /></div>
                    <div className="space-y-2"><label className="text-sm font-bold text-gray-700 flex items-center gap-2"><Phone size={16} /> Phone Number</label><input type="tel" value={formData.phoneNumber} onChange={(e) => setFormData({ ...formData, phoneNumber: e.target.value })} placeholder="Enter phone number" className="w-full rounded-xl border border-gray-200 px-4 py-3 focus:outline-none focus:ring-2 transition-all shadow-sm" style={{ '--tw-ring-color': '#7B4DDB33' }} /></div>
                  ...
                  <button type="submit" disabled={loading} style={{ backgroundColor: '#7B4DDB' }} className="w-full rounded-2xl py-4 font-bold text-white shadow-xl hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-50 mt-4">
